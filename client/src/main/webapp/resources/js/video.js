...
navigator.mediaDevices.getUserMedia({
	...
	.then((stream) => {
		...
		socket.on('user-connected', (_id) => {
			setTimeout(function() {
				connectToNewUser(_id, stream);
			}, 500);
		});
	});
...
peer.on('open', (_id) => {
	socket.emit('joinVideoRoom', _id, { userId, roomId });
	...
	chatForm.addEventListener('submit', (e) => {
			...
			socket.emit('videoChatMessage', _id, msg);
...
socket.on('user-disconnected', (_id) => {
	if (peers[_id]) peers[_id].close();
});


const connectToNewUser = (_id, stream) => {
